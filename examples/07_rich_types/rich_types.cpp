// The DataHandler types that do not cross into Python: Event, FunctionCall
// (with a handler registry) and ImageFrame, plus a mixed payload combining
// them with the scalar types.
//
// Demonstrates: createEvent / createFunctionCall / createImageFrame,
//               registerFunction and processFunctionCalls, extractEvents /
//               extractFunctions / extractImages, and encode/decode sizing.
//
// This one runs standalone - it encodes and decodes in the same process, with
// no channel and no peer. The Python codec in py/data_handler.py implements
// only the five scalar types, so these tags have no Python counterpart; see
// this directory README for what to do when Python is on the other end.
//
// Run:   ./rich_types
